...
2. **Subject**: Specify the exact product, person, object, character, environment, or interface.
3. **Composition**: Define canvas ratio, viewpoint, crop, layout grid, focal hierarchy, negative space, and foreground/midground/background.
4. **Scene and action**: Describe the moment, interaction, gesture, product use, sequence, or narrative.
5. **Camera and rendering**: Use medium-specific language: 35mm film, direct flash, telephoto editorial portrait, photorealistic CGI, engraved ink drawing, printed poster, isometric miniature, app screenshot.
6. **Lighting**: Name source, direction, intensity, color, contrast, shadows, reflections, rim light, bloom, bokeh, or volumetric effects.
7. **Materials and texture**: Specify surfaces such as matte packaging, brushed steel, glass, paper fiber, skin texture, fabric weave, food gloss, UI glassmorphism, ceramic glaze.
8. **Color system**: State dominant palette, accent colors, temperature, contrast, and brand-color limits.
9. **Text**: Give exact visible text, language, placement, hierarchy, and readability requirements. If no text, say no text.
10. **Constraints**: Add avoid rules for common failure modes.
